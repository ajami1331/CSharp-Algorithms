namespace Csharp_Contest
{
    ...
    public class PushRelabel
    {
        ...
        {
            this.nodes = nodes;
            this.adj = Enumerable.Repeat(0, nodes).Select(_ => new List<Edge>()).ToArray();
        ...
        {
            this.adj[from].Add(new Edge(from, to, capacity, 0, this.adj[to].Count));
            if (from == to)
            {
                this.adj[from].Last().index--;
            }

            this.adj[to].Add(new Edge(to, from, 0, 0, this.adj[from].Count - 1));
        }

        public long MaxFlow(int s, int t)
        {
            foreach (Edge e in this.adj[s])
            {
                this.excess[s] += e.capacity;
            }
        ...
        private void Push(Edge e)
        {
            ...
            {
                e.flow += amt;
                this.adj[e.to][e.index].flow -= amt;
        ...
        private void Relabel(int v)
        {
            ...
            foreach (Edge e in this.adj[v])
            {
                ...
                {
                    this.dist[v] = Math.Min(this.dist[v], this.dist[e.to] + 1);
                }
            }
